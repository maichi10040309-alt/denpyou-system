...
import { buildSalesSummary } from '../logic.js';

export async function renderSalesView(container) {
  const key = store.getCurrentMonth();
  const { year, month } = parseMonthKey(key);
  const entries = store.getEntries(key);
  const summary = buildSalesSummary(entries);

  container.innerHTML = `
    <h1 class="page-title">${year}年${month}月　売上集計表</h1>
    <div class="toolbar no-print">
      <button class="primary" id="btn-print">印刷する</button>
    </div>
    <div class="panel">
      <div class="summary-cards">
        <div class="card total"><div class="label">売上総合計</div><div class="value">¥${yen(summary.grandTotal)}</div></div>
